    ...
    public function extendWith(Menu $menu)
    {
        $menu->group(trans('core::sidebar.content'), function (Group $group) {
            $group->item(trans('news::news.title.news'), function (Item $item) {
                ...
                $item->item(trans('news::news_categories.title.news_categories'), function (Item $item) {
                    $item->icon('fa fa-copy');
                    $item->weight(0);
                    $item->append('admin.news.news_categories.create');
                    $item->route('admin.news.news_categories.index');
                    $item->authorize(
                        $this->auth->hasAccess('news.news_categories.index')
                    );
                });
