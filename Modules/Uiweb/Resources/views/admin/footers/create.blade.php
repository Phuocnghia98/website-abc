...
@section('content-header')
    <h1>
        {{ trans('uiweb::footers.title.create footer') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.uiweb.footer.index') }}">{{ trans('uiweb::footers.title.footers') }}</a></li>
        <li class="active">{{ trans('uiweb::footers.title.create footer') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.uiweb.footer.store'], 'method' => 'post', 'id' => 'form-footer']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('uiweb::admin.footers.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
